...
function renderDirt(){
  ...
  dirt.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>{if(store.state.cleanSpots){store.clean();void playSfx('cleanSwish');recordAction('clean')}}));
}
