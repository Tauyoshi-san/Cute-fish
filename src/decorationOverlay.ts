...
window.addEventListener('decor-action',e=>{const action=(e as CustomEvent<string>).detail,d=current();if(!d)return;const[min,max]=limits[d.type];if(action==='smaller')store.updateDecor(d.id,{scale:clamp(d.scale-.1,min,max)});if(action==='larger')store.updateDecor(d.id,{scale:clamp(d.scale+.1,min,max)});if(action==='rotate')store.updateDecor(d.id,{rotation:(d.rotation+15)%360});if(action==='flip')store.updateDecor(d.id,{flipped:!d.flipped});if(action==='substrate-back')store.updateDecor(d.id,{z:-10});if(action==='back')store.updateDecor(d.id,{z:clamp(d.z-1,-20,20)});if(action==='front')store.updateDecor(d.id,{z:clamp(d.z+1,-20,20)});if(action==='store'){store.storeDecor(d.id);selected=null;announce()}render()});
