...
function fishPanel() {
  ...
  return `<div class="console-section fish-summary">
    ...
    <div class="fish-title"><img src="/assets/sprites/fish/seeds/${fish.species}.png" alt=""><h2>${fish.name}<small>${SPECIES[fish.species].name}</small></h2></div>
  ...
  </div><div class="console-section"><h3>ごはんをあげる</h3><div class="cards">${(Object.keys(FOODS) as FoodId[]).map(id => `<button class="food" data-food="${id}" ${store.state.inventory[id] ? '' : 'disabled'}><i style="--c:#${FOODS[id].color.toString(16)}">${FOODS[id].icon}</i><b>${FOODS[id].name}</b><span>所持 ${store.state.inventory[id]}</span></button>`).join('')}</div><p class="history">食べたもの：${fish.foods.length ? fish.foods.map(id => FOODS[id].icon).join('　') : 'まだありません'}</p><button class="sell" ${store.state.fish.length === 1 ? 'disabled' : ''}>この魚を ${SPECIES[fish.species].value} コインで送り出す</button></div>`;
}

function shopPanel() {
  return `<div class="console-section"><p class="eyebrow">AQUARIUM SHOP</p><h2>水辺ショップ<small>ごはんと配置アイテム</small></h2><h3>ごはん</h3><div class="shopgrid">${(Object.keys(FOODS) as FoodId[]).map(id => `<button data-buyfood="${id}"><b>${FOODS[id].icon} ${FOODS[id].name}</b><span>${FOODS[id].desc}</span><em>${FOODS[id].cost} コイン</em></button>`).join('')}</div><h3>水槽アイテム</h3><div class="shopgrid">${SHOP.map(item => `<button data-decor="${item.type}"><b>${item.icon} ${item.name}</b><span>${item.desc}</span><em>${item.cost} コイン</em></button>`).join('')}</div><button class="primary" data-fish>稚魚を迎える · 55 コイン</button></div>`;
}

function bookPanel() {
  return `<div class="console-section"><p class="eyebrow">AQUA ARCHIVE</p><h2>いきもの図鑑<small>進化を見つけて記録しよう</small></h2><div class="species">${(Object.keys(SPECIES) as (keyof typeof SPECIES)[]).map(id => { const got = store.state.discovered.includes(id), item = SPECIES[id]; return `<article class="${got ? '' : 'locked'}"><img src="/assets/sprites/fish/seeds/${id}.png" alt=""><div><b>${got ? item.name : '？？？？？'}</b><span>${got ? item.hint : 'まだ見つかっていません'}</span></div><em>${got ? item.value + ' コイン' : '未発見'}</em></article>`; }).join('')}</div></div>`;
}

function homePanel(){return `<div class="console-section console-home"><p class="eyebrow">AQUARIUM STATUS</p><h2>水槽のお世話<small>魚をクリックすると、ごはん画面が開きます</small></h2><div class="home-fish">${store.state.fish.map(f=>`<button data-openfish="${f.id}"><img src="/assets/sprites/fish/seeds/${f.species}.png" alt=""><b>${f.name}</b><span>${SPECIES[f.species].name}</span></button>`).join('')}</div></div>`}
function decorPanel(){const placed=store.state.decorations.filter(d=>d.placed),stored=store.state.decorations.filter(d=>!d.placed);return `<div class="console-section decor-console"><p class="eyebrow">LAYOUT EDITOR</p><h2>配置編集<small>装飾を選ぶと、水槽下部に調整バーが開きます</small></h2><div class="decor-list">${placed.length?placed.map(x=>`<button data-pickdecor="${x.id}" class="${x.id===decorSelected?'active':''}"><span>${SHOP.find(i=>i.type===x.type)?.icon||'◇'}</span><b>${SHOP.find(i=>i.type===x.type)?.name||x.type}</b></button>`).join(''):'<p>配置中の装飾はありません</p>'}</div><p class="decor-guide">水槽内では装飾を上下左右へドラッグできます。</p>${stored.length?`<h3>収納中</h3><div class="decor-list">${stored.map(x=>`<button data-restoredecor="${x.id}"><span>${SHOP.find(i=>i.type===x.type)?.icon||'◇'}</span><b>${SHOP.find(i=>i.type===x.type)?.name||x.type}</b></button>`).join('')}</div>`:''}</div>`}
...
function bind(root:ParentNode=hud) {
  ...
  root.querySelector('[data-clean]')?.addEventListener('click', () => { store.clean(); toast('水槽がきれいになった！ +12 コイン'); });
  root.querySelectorAll<HTMLElement>('[data-food]').forEach(button => button.onclick = () => { const fishId=selected!,food=button.dataset.food as FoodId,origin=button.getBoundingClientRect(),evolved=store.feed(fishId,food);document.querySelector('.fish-dialog')?.remove();requestAnimationFrame(()=>window.dispatchEvent(new CustomEvent('feed-effect',{detail:{fishId,food,origin}})));if(evolved)toast(`進化！ ${SPECIES[evolved].name} になった`);setTimeout(()=>openFishDialog(fishId),1450); });
  ...
  root.querySelectorAll<HTMLElement>('[data-buyfood]').forEach(button => button.onclick = () => { const id = button.dataset.buyfood as FoodId; store.buyFood(id, FOODS[id].cost) ? toast(`${FOODS[id].name}を購入`) : toast('コインが足りません'); });
  root.querySelectorAll<HTMLElement>('[data-decor]').forEach(button => button.onclick = () => { const item = SHOP.find(value => value.type === button.dataset.decor)!; store.addDecor(item.type, item.cost) ? toast(`${item.name}を設置しました`) : toast('コインが足りません'); });
  root.querySelector('[data-fish]')?.addEventListener('click', () => store.buyFish() ? toast('新しい稚魚が仲間入り！') : toast('コイン不足、または水槽が満員です'));
