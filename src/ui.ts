...
function bookPanel() {
  ...
  return `<div class="console-section"><p class="eyebrow">AQUA ARCHIVE</p><div class="archive-title"><h2>いきもの図鑑<small>進化を見つけて記録しよう</small></h2><strong><b>${completion}%</b><span>${discovered}/${ids.length}</span></strong></div><div class="species">${ids.map(id => { const got = store.state.discovered.includes(id), item = SPECIES[id]; return `<article class="${got ? '' : 'locked'}">${got?`<img src="${fishAsset(id)}" alt="${item.name}">`:'<span class="unknown-fish" aria-label="未発見の魚">？</span>'}<div><b>${got ? item.name : '？'}</b><span>${got ? item.hint : `ヒント：${item.discoveryHint}`}</span></div><em>${got ? item.value + ' コイン' : '未発見'}</em></article>`; }).join('')}</div></div>`;
}
