...
function bookPanel() {
  return `<div class="console-section"><p class="eyebrow">AQUA ARCHIVE</p><h2>いきもの図鑑<small>進化を見つけて記録しよう</small></h2><div class="species">${(Object.keys(SPECIES) as (keyof typeof SPECIES)[]).map(id => { const got = store.state.discovered.includes(id), item = SPECIES[id]; return `<article class="${got ? '' : 'locked'}"><img src="${fishAsset(id)}" alt=""><div><b>${got ? item.name : '？？？？？'}</b><span>${got ? item.hint : 'まだ見つかっていません'}</span></div><em>${got ? item.value + ' コイン' : '未発見'}</em></article>`; }).join('')}</div></div>`;
}
