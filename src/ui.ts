import {BACKGROUNDS,FOODS, SHOP, SPECIES, SUBSTRATES} from './data';
import {store} from './store';
import type {BackgroundId,FoodId,SubstrateId} from './types';
...
function render() {
  ...
  document.querySelector<HTMLElement>('#camera-stage')!.dataset.background=state.background;
  const audio=getAudioSettings();
  hud.innerHTML = `<aside class="tank-console"><div class="console-head"><div class="resources"><span class="help-tip" tabindex="0" data-tooltip="ショップで餌や水槽アイテムを購入するためのコインです。">🪙 ${state.coins}</span><span class="help-tip" tabindex="0" data-tooltip="特別なごほうびとして集める、貴重なパールです。">🫧 ${state.pearls}</span><span class="water-gauge help-tip" tabindex="0" data-tooltip="水槽のきれいさです。魚のごきげんや進化条件に影響します。" aria-label="水質 ${Math.round(state.water)}"><b>水質</b>${bar(state.water)}</span></div><div class="sound-control"><button class="help-tip" data-tooltip="水槽のBGMをオン・オフします。" data-mute aria-label="BGMを${audio.muted?'オン':'オフ'}">${audio.muted?'🔇':'🔊'}</button><label class="help-tip" tabindex="0" data-tooltip="水槽内で流れる環境BGMの音量です。"><span>BGM</span><input data-volume type="range" min="0" max="100" value="${Math.round(audio.volume*100)}" aria-label="BGM音量"><output>${Math.round(audio.volume*100)}%</output></label></div></div><nav class="console-tabs"><button data-tab="home" class="${panel === 'home' ? 'active' : ''}">🐟<small>お世話</small></button><button data-tab="shop" class="${panel === 'shop' ? 'active' : ''}">🛒<small>ショップ</small></button><button data-tab="book" class="${panel === 'book' ? 'active' : ''}">⭐<small>図鑑</small></button><button data-tab="decor" class="${panel === 'decor' ? 'active' : ''}">✥<small>配置</small></button></nav><div class="console-body">${panel==='home'?homePanel():panel==='shop'?shopPanel():panel==='book'?bookPanel():decorPanel()}</div><div class="console-actions"><button data-clean ${state.cleanSpots ? '' : 'disabled'}>✨ お掃除 ${state.cleanSpots}</button><button data-ad>🎁 ごほうび</button></div></aside>`;
...
function bind(root:ParentNode=hud) {
  ...
  root.querySelectorAll<HTMLElement>('[data-background]').forEach(button=>button.onclick=()=>{const id=button.dataset.background as BackgroundId,item=BACKGROUNDS[id],owned=store.state.ownedBackgrounds.includes(id);if(store.buyBackground(id,item.cost)){void playSfx(owned?'tap':'buySuccess');if(!owned)recordAction('buy');toast(`${item.name}に変更しました`)}else{void playSfx('fail');toast('コインが足りません')}});
