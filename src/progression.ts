...
export function recordAction(action:ProgressAction){const value=getProgress(),mission=getMission(value);value[action]++;if(!achieved(value,mission)){save(value);return}const reward=mission.reward;value.completed++;const previous=value.missionId;Object.assign(value,fresh(value.tutorialDone,previous),{completed:value.completed});save(value);store.grantCoins(reward);window.dispatchEvent(new CustomEvent('progressreward',{detail:{reward,title:mission.title}}))}
