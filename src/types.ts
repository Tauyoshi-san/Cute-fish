...
export interface GameState{schemaVersion:number;coins:number;pearls:number;water:number;cleanSpots:number;fish:Fish[];decorations:Decoration[];inventory:Record<FoodId,number>;discovered:SpeciesId[];substrate:SubstrateId;ownedSubstrates:SubstrateId[];lastTick:number}
