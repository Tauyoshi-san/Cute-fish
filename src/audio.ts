...
type Sfx='tap'|'feed'|'buy'|'clean'|'reward'|'evolve'|'place';
let audioContext:AudioContext|undefined;
export async function playSfx(kind:Sfx){if(settings.sfxVolume<=0)return;audioContext??=new AudioContext();const ctx=audioContext;if(ctx.state==='suspended')await ctx.resume();const now=ctx.currentTime,gain=ctx.createGain(),osc=ctx.createOscillator();const tones:Record<Sfx,[number,number,OscillatorType,number]>={tap:[520,660,'sine',.09],feed:[420,820,'sine',.18],buy:[620,1040,'triangle',.2],clean:[760,1380,'sine',.25],reward:[560,1320,'triangle',.32],evolve:[320,1520,'sine',.55],place:[240,440,'triangle',.14]};const[start,end,type,duration]=tones[kind];osc.type=type;osc.frequency.setValueAtTime(start,now);osc.frequency.exponentialRampToValueAtTime(end,now+duration);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(Math.max(.002,settings.sfxVolume*.28),now+.012);gain.gain.exponentialRampToValueAtTime(.0001,now+duration);osc.connect(gain).connect(ctx.destination);osc.start(now);osc.stop(now+duration+.02)}
...
document.addEventListener('pointerdown',event=>{const el=(event.target as HTMLElement).closest<HTMLButtonElement>('button');if(!el||el.disabled)return;if(el.matches('[data-food]'))void playSfx('feed');else if(el.matches('[data-buyfood],[data-decor],[data-fish]'))void playSfx('buy');else if(el.matches('[data-clean]'))void playSfx('clean');else if(el.matches('[data-ad],.ad button'))void playSfx('reward');else if(el.matches('[data-decoract],[data-act],[data-pickdecor],[data-restoredecor]'))void playSfx('place');else void playSfx('tap')});
